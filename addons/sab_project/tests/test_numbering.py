...
class TestSabNumbering(TransactionCase):
    ...
    def test_default_bearbeiter_and_classification(self):
        ...
        self.assertEqual(
            project.sab_inquiry_type_id,
            self.env.ref("sab_project.sab_inquiry_type_electrician"),
        )
        self.assertEqual(
            project.sab_technology_id,
            self.env.ref("sab_project.sab_technology_energy_distribution"),
        )
        self.assertEqual(
            project.sab_service_type_id,
            self.env.ref("sab_project.sab_service_type_new_installation"),
        )
    ...
    def test_create_quotation_action_prefills_project_and_customer(self):
        ...
        quotation = self.env["sale.order"].with_context(**action["context"]).create({})
    ...
    def test_legacy_project_gets_number_on_first_sab_use(self):
        ...
        self.env.cr.execute(
            "UPDATE project_project SET sab_project_reference = NULL WHERE id = %s",
            [project.id],
        )
