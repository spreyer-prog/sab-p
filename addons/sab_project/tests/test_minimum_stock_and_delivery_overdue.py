...
class TestSabMinimumStockAndDeliveryOverdue(TransactionCase):
    ...
    def _create_requirement(self, minimum_stock=5.0):
        self.product.write({"minimum_stock_quantity": minimum_stock})
        ...
        total_bom = self.env["sab.project.bom"].create(
            {
                "name": "Gesamtstückliste Mindestbestand",
                "order_id": self.sale_order.id,
                "project_id": self.project.id,
                "bom_scope": "total",
                "line_ids": [(0, 0, line_vals)],
    ...
    def _create_purchase_order(self, include_minimum=False, minimum_stock=5.0):
        package, requirement = self._create_requirement(minimum_stock=minimum_stock)
        requirement.write(
            {"include_minimum_stock_replenishment": include_minimum}
        )
        requirement._sab_prepare_order_quantities(force=True)
        action = requirement.action_create_purchase_orders()
        order_ids = action["domain"][0][2]
        purchase_order = self.env["sab.purchase.order"].browse(order_ids)
    ...
    def test_supplier_packaging_and_minimum_only_create_warning(self):
        _package, requirement = self._create_requirement()
        self.assertEqual(requirement.quantity_to_order, 6.0)
        self.assertIn("Mindestbestellmenge", requirement.supplier_quantity_warning)
        self.assertIn("Verpackungseinheit", requirement.supplier_quantity_warning)
    ...
    def test_purchase_order_uses_exact_shortage_only_by_default(self):
        ...
        self.assertEqual(requirement.quantity_to_order, 6.0)
        self.assertEqual(purchase_order.line_ids.quantity_ordered, 6.0)

    def test_minimum_stock_is_added_only_after_explicit_selection(self):
        _package, requirement = self._create_requirement()
        self.assertTrue(requirement.minimum_stock_below)
        self.assertGreater(requirement.minimum_stock_replenishment_quantity, 0.0)
        shortage = requirement.shortage_quantity
        replenishment = requirement.minimum_stock_replenishment_quantity
    ...
    def test_no_minimum_stock_means_only_real_shortage_can_be_ordered(self):
        _package, requirement = self._create_requirement(minimum_stock=0.0)
        self.assertFalse(requirement.minimum_stock_below)
        self.assertEqual(requirement.minimum_stock_replenishment_quantity, 0.0)
        requirement.write({"include_minimum_stock_replenishment": True})
        requirement._sab_prepare_order_quantities(force=True)
        self.assertEqual(requirement.quantity_to_order, requirement.shortage_quantity)
        self.assertEqual(requirement.quantity_to_order, 6.0)

    def test_explicit_minimum_stock_selection_is_used_in_purchase_order(self):
    ...
    def test_overdue_delivery_is_listed_and_supplier_can_be_reminded(self):
        ...
        purchase_order.action_submit_for_approval()
        purchase_order.action_approve()
        purchase_order.write({"state": "sent"})
        purchase_order.line_ids.write(
            {"expected_delivery_date": fields.Date.today() - timedelta(days=2)}
        )
        overdue = self.env["sab.purchase.order.line"].search(
            [
                ("id", "in", purchase_order.line_ids.ids),
    ...
    def test_overdue_delivery_view_contains_order_and_position_details(self):
        view = self.env.ref("sab_project.view_sab_overdue_delivery_line_list")
        arch = view.arch_db
        self.assertIn("order_id", arch)
