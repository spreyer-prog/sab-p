...
class TestSabDatanormImport(TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.supplier = cls.env["sab.supplier"].create({"name": "ABB DATANORM Test"})
    ...
    def test_zip_prefers_abb_shorttext_with_type(self):
        ...
        with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as archive:
            archive.writestr("DataNorm5_2026-08_Direkt-Gesamt.001", "\n".join([HEADER, ARTICLE, END]))
            archive.writestr("DataNorm5_2026-08_Direkt_KurztextinklTyp-Gesamt.001", "\n".join([HEADER, ARTICLE_WITH_TYPE, END]))
