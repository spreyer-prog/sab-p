...
class TestSabSecurityPermissions(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.normal_profile = cls.env["sab.employee.profile"].create({"name": "SAB Mitarbeiter ohne Freigabe", "login": cls.normal_user.login, "user_id": cls.normal_user.id, "work_area_ids": [(6, 0, all_areas)], "mobile_access": True})
        cls.other_profile = cls.env["sab.employee.profile"].create({"name": "SAB anderer Mitarbeiter", "login": cls.other_employee.login, "user_id": cls.other_employee.id, "work_area_ids": [(6, 0, all_areas)], "mobile_access": True})
        ...
        cls.own_step, cls.other_step, cls.free_step = steps[0], steps[1], steps[2]
    ...
    def test_non_employee_internal_user_has_no_employee_workflow_access(self):
        self.assertFalse(self.env["sab.production.step"].with_user(self.release_user).has_access("read"))
        self.assertFalse(self.env["sab.time.entry"].with_user(self.release_user).has_access("read"))
        self.assertFalse(self.env["sab.employee.feedback"].with_user(self.release_user).has_access("read"))

    def test_employee_can_read_only_own_profile(self):
        self.assertTrue(self.normal_profile.with_user(self.normal_user).has_access("read"))
        self.assertFalse(self.other_profile.with_user(self.normal_user).has_access("read"))
    ...
    def test_release_user_can_publish_customer_content(self):
        self.status.with_user(self.release_user).action_release(); self.assertTrue(self.status.released)
        self.document.with_user(self.release_user).action_release_to_customer(); self.assertTrue(self.document.customer_visible)

    def test_employee_sees_only_own_or_matching_free_steps(self):
        visible = self.env["sab.production.step"].with_user(self.normal_user).search([("production_order_id", "=", self.production.id)])
        self.assertIn(self.own_step, visible); self.assertIn(self.free_step, visible); self.assertNotIn(self.other_step, visible)

    def test_employee_cannot_read_other_employee_step_directly(self):
        self.assertFalse(self.other_step.with_user(self.normal_user).has_access("read"))
