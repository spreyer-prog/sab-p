...
class TestSabUiContracts(TransactionCase):
    ...
    def _assert_button_label(self, view_xmlid, button_name, expected_label):
        ...
        matches = arch.xpath(
            f"//button[@name='{button_name}'][@string='{expected_label}']"
        )
        self.assertTrue(
            matches,
    ...
    def test_standard_odoo_project_list_is_extended_with_project_number(self):
        ...
        inserted = arch.xpath(
            "//field[@name='name']/field[@name='sab_project_reference']"
        )
    ...
    def test_employee_admin_exposes_invitation_and_access_status(self):
        ...
        arch = etree.fromstring(view.arch_db.encode("utf-8"))
        self.assertEqual(
            len(arch.xpath("//button[@name='action_send_invitation']")),
            1,
        )
    ...
    def test_production_ui_uses_work_area_and_employee_profile(self):
        ...
        arch = etree.fromstring(view.arch_db.encode("utf-8"))
        self.assertTrue(
            arch.xpath("//field[@name='step_ids']//field[@name='work_area_id']")
        )
        self.assertTrue(
            arch.xpath(
                "//field[@name='step_ids']//field[@name='responsible_employee_id']"
            )
        )
    ...
    def test_employee_mobile_ui_uses_employee_profile(self):
        ...
        self.assertEqual(
            len(arch.xpath("//button[@name='action_open_time_entry']")),
            1,
        )
        self.assertEqual(
            len(arch.xpath("//button[@name='action_open_feedback']")),
            1,
        )
    ...
    def test_controlling_action_exposes_list_pivot_graph_and_form(self):
        ...
        list_arch = etree.fromstring(
            self.env.ref(
                "sab_project.view_sab_project_controlling_list"
            ).arch_db.encode("utf-8")
        )
        pivot_arch = etree.fromstring(
            self.env.ref(
                "sab_project.view_sab_project_controlling_pivot"
            ).arch_db.encode("utf-8")
        )
        graph_arch = etree.fromstring(
            self.env.ref(
                "sab_project.view_sab_project_controlling_graph"
            ).arch_db.encode("utf-8")
        )

        self.assertTrue(
            list_arch.xpath("/list/field[@name='contribution_margin']")
        )
        self.assertTrue(
            list_arch.xpath(
                "/list/field[@name='contribution_margin_percent']"
            )
        )
        for measure in (
            "offer_amount",
            ...
            self.assertTrue(
                pivot_arch.xpath(
                    f"/pivot/field[@name='{measure}'][@type='measure']"
                )
            )
        for measure in (
            "offer_amount",
            ...
            self.assertTrue(
                graph_arch.xpath(
                    f"/graph/field[@name='{measure}'][@type='measure']"
                )
            )
    ...
    def test_sale_portal_extension_keeps_odoo_container_in_dom_and_renders_only_component_sub_lines(self):
        ...
        self.assertTrue(
            item_templates[0].xpath(
                ".//td[contains(@t-att-style, 'padding-left')]"
            )
        )

        backend = self.env.ref(
            "sab_project.sab_sale_order_form_switchboard_rules"
        )
    ...
    def test_primary_button_labels_match_documented_ui_contract(self):
        ...
        expected = (
            (
                "sab_project.sab_project_project_form_inherit",
                "action_create_sab_quotation",
                "Neues Angebot",
            ),
            (
                "sab_project.sab_project_project_form_inherit",
                "action_view_sab_quotations",
                "Angebote",
            ),
            (
                "sab_project.sab_project_project_form_inherit",
                "action_assign_sab_project_reference",
                "Projektnummer vergeben",
            ),
            (
                "sab_project.sab_sale_order_form_inherit",
                "action_create_sab_revision",
                "Neue Revision",
            ),
            (
                "sab_project.sab_sale_order_form_inherit",
                "action_generate_sab_bom",
                "Stückliste erzeugen",
            ),
            (
                "sab_project.view_sab_project_bom_form",
                "action_release",
                "Stückliste freigeben",
            ),
            (
                "sab_project.view_sab_project_bom_form",
                "action_create_production_order",
                "Fertigungsauftrag erzeugen",
            ),
            (
                "sab_project.view_sab_project_bom_form_purchase_release",
                "action_release_for_purchase",
                "Zur Beschaffung freigeben",
            ),
            (
                "sab_project.view_sab_purchase_requirement_list",
                "action_create_purchase_orders",
                "Bestellung aus Auswahl erzeugen",
            ),
            (
                "sab_project.view_sab_purchase_order_form",
            ...
            (
                "sab_project.view_sab_purchase_order_form",
                "action_post_receipts",
                "Wareneingang buchen",
            ),
            (
                "sab_project.view_sab_production_order_form",
                "action_start",
                "Fertigung starten",
            ),
            (
            ...
            (
                "sab_project.view_sab_employee_profile_form",
                "action_create_or_update_user",
                "Zugang anlegen / aktualisieren",
            ),
            (
                "sab_project.view_sab_employee_profile_form",
                "action_send_invitation",
                "Einladung / Passwortlink senden",
            ),
            (
                "sab_project.view_sab_employee_profile_form",
                "action_apply_permissions",
                "Rechte übernehmen",
            ),
            (
                "sab_project.view_sab_employee_profile_form",
            ...
            (
                "sab_project.view_sab_employee_step_form",
                "action_claim",
                "Arbeit übernehmen",
            ),
            (
            ...
            (
                "sab_project.view_sab_employee_step_form",
                "action_open_time_entry",
                "Zeit erfassen",
            ),
            (
                "sab_project.view_sab_employee_step_form",
                "action_open_feedback",
                "Rückmeldung / Foto / Material",
            ),
            (
                "sab_project.view_sab_employee_step_form",
                "action_done",
            ...
            (
                "sab_project.view_sab_employee_time_entry_form",
                "action_confirm",
                "Zeit buchen",
            ),
            (
                "sab_project.view_sab_project_document_form",
                "action_release",
                "Dokument intern freigeben",
            ),
            (
                "sab_project.view_sab_project_document_form",
                "action_release_to_customer",
                "Für Kundenportal freigeben",
            ),
            (
                "sab_project.view_sab_project_document_form",
                "action_withdraw_customer_release",
                "Kundenfreigabe zurückziehen",
            ),
            (
                "sab_project.view_sab_project_document_form",
            ...
            (
                "sab_project.view_sab_employee_feedback_form",
            ...
            (
                "sab_project.view_sab_customer_status_form",
                "action_apply_suggestion",
                "Internen Stand übernehmen",
            ),
            (
                "sab_project.view_sab_customer_status_form",
                "action_release",
            ...
            (
                "sab_project.view_sab_datanorm_import_form",
                "action_import",
