    ...
    def action_import(self):
        ...
        for raw_line in lines[1:]:
            ...
            try:
                ...
                vals_product = {"manufacturer_id": manufacturer.id, "manufacturer_article_number": manufacturer_article, "datanorm_number": article_number, "name": short_text or type_name or manufacturer_article}
                if product:
                    ...
                    product = Product.create(vals_product); product_map[manufacturer_article] = product; created_products += 1
                else:
                    skipped += 1
                    continue

                supplier_product = supplier_map.get(article_number)
                vals_supplier = {"supplier_id": self.supplier_id.id, "product_id": product.id, "supplier_article_number": article_number, "datanorm_number": article_number, "datanorm_price": datanorm_price, "list_price": datanorm_price, "datanorm_price_code": datanorm_price_code, "unit": self._unit_from_datanorm(unit), "valid_from": source_date, "datanorm_type_name": type_name, "ean": ean}
                ...
                if supplier_product:
                    ...
                    supplier_product = SupplierProduct.create(vals_supplier); supplier_map[article_number] = supplier_product; created_supplier += 1

                # Nach Anlage/Aktualisierung des Lieferantenartikels ist der kalkulatorische EK vollständig.
        ...
        self.result_text = (
            ...
            f"A-Artikelsätze erkannt: {record_counts.get('A', 0)}\nZ-Preis-/Zuschlagssätze erkannt: {record_counts.get('Z', 0)} (noch nicht preiswirksam importiert)\n\n"
            ...
            f"Mit Odoo-Produktstamm synchronisiert: {synced_odoo}\nÜbersprungen: {skipped}\nFehler: {errors}"
        )
