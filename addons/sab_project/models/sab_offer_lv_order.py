    ...
    def _sab_validate_and_lock_lv(self):
        for order in self:
            ...
            visible = order.sab_calculation_line_ids.filtered(
                ...
                or (line.line_type == "item" and not line.parent_section_id)
            )
            ...
            if missing:
                names = ", ".join(
                    (
                        line.description
                        or line.calculation_item_id.name
                ...
                raise ValidationError(
                    "Bei einem LV-Angebot benötigt jede im Kundenangebot sichtbare "
                    "Position eine LV-Position. Neue Nachtragspositionen müssen als "
                    "NTG geführt werden. Fehlend bei: "
                    + names
                )
