...
class SabProjectLvMapping(models.Model):
    ...
    position_code = fields.Char(string="LV-/NTG-Position", required=True, index=True)
    ...
    first_order_id = fields.Many2one("sale.order", string="Erstmals verwendet in", ondelete="set null", readonly=True)

    @api.constrains("calculation_item_id", "odoo_product_id")
    def _check_source(self):
        for record in self:
            if bool(record.calculation_item_id) == bool(record.odoo_product_id):
                raise ValidationError("Eine LV-Zuordnung muss genau einem Kalkulationsartikel oder einem Produkt zugeordnet sein.")

    @api.constrains("project_id", "calculation_item_id", "odoo_product_id", "position_code")
    def _check_unique_mapping(self):
        for record in self:
            domain = [("project_id", "=", record.project_id.id), ("id", "!=", record.id)]
            if record.calculation_item_id:
                domain.append(("calculation_item_id", "=", record.calculation_item_id.id))
            else:
                domain.append(("odoo_product_id", "=", record.odoo_product_id.id))
            if self.search_count(domain):
                raise ValidationError("Für diesen Artikel existiert im Projekt bereits eine feste LV-/NTG-Zuordnung.")

    def write(self, vals):
        immutable = {"project_id", "calculation_item_id", "odoo_product_id", "position_code", "is_ntg", "ntg_number"}
        if immutable & set(vals):
            for record in self:
                for field_name in immutable & set(vals):
                    ...
                    if incoming != current:
                        raise ValidationError("Eine einmal festgelegte projektbezogene LV-/NTG-Zuordnung darf nicht geändert werden.")
