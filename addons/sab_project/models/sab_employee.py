...
class SabWorkArea(models.Model):
    ...
    employee_ids = fields.Many2many(comodel_name="sab.employee.profile", relation="sab_employee_work_area_rel", column1="work_area_id", column2="employee_id", string="Freigegebene Mitarbeiter", readonly=True)
...
class SabEmployeeProfile(models.Model):
    ...
    def _check_employee_admin(self):
        if not self.env.user.has_group("project.group_project_manager"):
            raise AccessError(_("Nur die SAB-P Projektleitung darf Mitarbeiterzugänge und Berechtigungen verwalten."))
    ...
    def action_create_or_update_user(self):
        ...
        if not user:
            ...
            if existing:
                ...
                user = self.env["res.users"].sudo().with_context(no_reset_password=True).create({"name": self.name, "login": self.login, "email": self.email or False, "active": True, "group_ids": [Command.link(self.env.ref("base.group_user").id)]})
            self.user_id = user
