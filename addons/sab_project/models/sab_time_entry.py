...
class SabTimeEntry(models.Model):
    ...
    activity_type = fields.Selection(selection=[("engineering", "Planung / Technik"), ("mechanical", "Mechanische Fertigung"), ("wiring", "Verdrahtung"), ("testing", "Prüfung"), ("service", "Service / Montage"), ("travel", "Fahrtzeit"), ("documentation", "Dokumentation"), ("other", "Sonstiges")], string="Tätigkeitsart", required=True, default="service", index=True)
    hours = fields.Float(string="Stunden", required=True, digits=(16, 2), tracking=True)
    hourly_cost = fields.Float(string="Kostensatz / h", required=True, digits=(16, 2), default=lambda self: self._default_hourly_cost(), tracking=True, groups="project.group_project_manager")
    cost_total = fields.Float(string="Kosten gesamt", compute="_compute_cost_total", store=True, digits=(16, 2), groups="project.group_project_manager")
    ...
    def _default_hourly_cost(self):
        ...
        except (TypeError, ValueError): return 80.0
    ...
    def _compute_cost_total(self):
        for record in self: record.cost_total = (record.hours or 0.0) * (record.hourly_cost or 0.0)

    @api.constrains("hours", "hourly_cost")
    def _check_values(self):
        for record in self:
            if record.hours <= 0: raise ValidationError(_("Eine Zeitbuchung benötigt eine Dauer größer 0 Stunden."))
            if record.hourly_cost < 0: raise ValidationError(_("Der Kostensatz darf nicht negativ sein."))
    ...
    def _check_production_step_context(self):
        for record in self.filtered("production_step_id"):
            ...
            if record.project_id != step.project_id: raise ValidationError(_("Projekt und Fertigungsschritt der Zeitbuchung passen nicht zusammen."))
    ...
    def create(self, vals_list):
        ...
        for incoming in vals_list:
            ...
            if not is_manager and vals.get("user_id") and vals["user_id"] != self.env.user.id:
                raise AccessError(_("Mitarbeiter dürfen Zeitbuchungen nur für sich selbst erfassen."))
            step_id = vals.get("production_step_id")
            if step_id:
                step = self.env["sab.production.step"].browse(step_id).exists()
                if step:
                    vals.setdefault("project_id", step.project_id.id); vals.setdefault("name", step.name)
            if not is_manager:
                vals["user_id"] = self.env.user.id
            else:
                vals.setdefault("user_id", self.env.user.id)
            prepared.append(vals)
        records = super().create(prepared); self._sync_project_hours(records.mapped("project_id")); return records

    def action_confirm(self):
        for record in self:
            if record.state == "draft": record.state = "confirmed"
        self._sync_project_hours(self.mapped("project_id")); return True
    ...
    def write(self, vals):
        ...
        if any(record.state == "confirmed" for record in self) and protected.intersection(vals): raise ValidationError(_("Gebuchte Zeiten dürfen nicht nachträglich verändert werden."))
        if not self.env.user.has_group("project.group_project_manager") and vals.get("user_id") and vals["user_id"] != self.env.user.id:
            raise AccessError(_("Mitarbeiter dürfen Zeitbuchungen nicht auf andere Benutzer übertragen."))
        result = super().write(vals); self._sync_project_hours(projects_before | self.mapped("project_id")); return result

    def unlink(self):
        if any(record.state == "confirmed" for record in self): raise ValidationError(_("Gebuchte Zeiten dürfen nicht gelöscht werden."))
        projects = self.mapped("project_id"); result = super().unlink(); self._sync_project_hours(projects); return result
